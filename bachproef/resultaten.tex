...
De \textit{LLM-as-a-Judge} kent deze scores toe op basis van een vaste beoordelingsprompt waarin per testcase de verwachte feiten, de vereiste outputstructuur en de beoordelingscriteria expliciet worden meegegeven. De judge vergelijkt de gegenereerde output dus niet alleen semantisch met de officiële changelog, maar beoordeelt ook of de response voldoende specifiek, volledig en actiegericht is volgens een vooraf vastgelegde rubric. De score is bijgevolg een modelgestuurde interpretatie van inhoudelijke overeenkomst en bruikbaarheid, en geen eenvoudige trefwoordtelling.

Het model werd dus niet apart getraind op deze benchmark, maar beoordeelde elke output op basis van de meegegeven instructies, verwachte feiten en scoringrubric.
...
\begin{table}[H]
    \centering
    \small
    \begin{tabular}{p{1.2cm} p{2.0cm} p{3.0cm} p{1.2cm} p{6.0cm}}
        \hline
        \textbf{ID} & \textbf{Package} & \textbf{Versieovergang} & \textbf{Type} & \textbf{Verwacht oordeel \& kernpunten} \\
        \hline
        minor & react & 18.2.0 → 18.3.0 & minor & \textit{Yes} — alleen deprecatiewaarschuwingen; geen nieuwe functies \\
        minor & nextjs & 14.1.0 → 14.2.0 & minor & \textit{Yes} — Turbopack-verbeteringen, drastische geheugenreductie \\
        major & typescript & 4.9.5 → 5.0.4 & major & \textit{With caution} — nieuwe decorators, deprecated tsconfig-opties \\
        patch & zod & 3.22.3 → 3.22.4 & patch & \textit{Yes} — geen publieke patchnotes; eerlijkheidstest \\
        minor & tailwindcss & 3.3.0 → 3.4.0 & minor & \textit{Yes} — uitgebreide nieuwe utilities, volledig compatibel \\
        \hline
    \end{tabular}
...
\subsection{Statistische vergelijking van de agents}
\label{subsec:statistische-vergelijking}

Om na te gaan of de waargenomen verschillen tussen de drie agents ook statistisch relevant zijn, werd per scorecategorie een one-way ANOVA uitgevoerd met \textit{agent} als factor. Deze analyse werd apart toegepast op de overall score, specificiteit, volledigheid, actiegerichtheid en latency. De factor \textit{testcase} werd in deze evaluatie niet expliciet gemodelleerd, waardoor de resultaten vooral als een eerste indicatie van groepsverschillen moeten worden geïnterpreteerd.

Voor de overall score werd een significant verschil gevonden tussen de agents (\(F(2,42)=5{,}41\), \(p=0{,}0081\)). Voor specificiteit werd eveneens een significant verschil gevonden (\(F(2,42)=8{,}82\), \(p=0{,}00063\)). Ook voor volledigheid was het verschil statistisch significant (\(F(2,42)=8{,}80\), \(p=0{,}00064\)). Voor actiegerichtheid werd geen statistisch significant verschil gevonden (\(F(2,42)=2{,}11\), \(p=0{,}134\)). Voor latency was het verschil duidelijk significant (\(F(2,42)=26{,}97\), \(p<0{,}001\)).

Omdat de benchmarkscore per testcase discreet is en de testcases inhoudelijk sterk verschillen, moet deze analyse voorzichtig worden geïnterpreteerd. Een two-way ANOVA of mixed model met \textit{testcase} als bijkomende factor zou methodologisch strikter zijn, maar ligt buiten de scope van deze bachelorproef. De huidige analyse volstaat wel om de dominante trends in de benchmarkresultaten te onderbouwen.
...
\textbf{Resultaten:} Mastra domineert duidelijk met een gemiddelde overallscore van 8,85 -- de hoogste score voor enige agent op enige testcase. Mastra haalt live changelog-data op via de \texttt{fetch-npm-info}-tool en slaagt erin de meeste verwachte utilities correct te benoemen. OpenAI (gem.\ 5,48) mist het overgrote deel van de nieuwe functies en noemt ten onrechte breaking changes. n8n (gem.\ 6,02) vermeldt in twee runs gefabriceerde details over een ``devtools-integratie'' die niet in de officiële changelog staat.
...
De benchmarkresultaten tonen aan dat de kwaliteit van AI-gestuurde dependencyanalyse sterk afhangt van de implementatievorm. Hoewel Mastra en OpenAI gebruikmaken van dezelfde ondersteunende tools, verschilt de manier waarop de opgehaalde informatie wordt georkestreerd en verwerkt. Mastra scoort vooral beter op specificiteit, volledigheid en actiegerichtheid, terwijl OpenAI sneller werkt en in sommige gevallen consistente output levert, maar minder diepgaande en minder accurate samenvattingen produceert bij updates met veel informatie.

De statistische vergelijking bevestigt dit algemene beeld: de verschillen tussen de agents zijn niet enkel descriptief, maar ook zichtbaar op meerdere scorecategorieën. Vooral de combinatie van inhoudelijke kwaliteit en verwerkingstijd laat zien dat de keuze voor een implementatie een duidelijke afweging inhoudt tussen nauwkeurigheid, volledigheid en snelheid.

Een opvallende bevinding is dat alle varianten het best presteren op de \texttt{zod\_patch}-testcase, niet omdat die inhoudelijk het rijkst is, maar omdat het ontbreken van publieke patchnotes merkwaardig genoeg de hallucinatiedruk verlaagt: er is minder ruimte om foutieve details te introduceren. De moeilijkste testcase blijkt de \texttt{nextjs\_minor}-update, waar alle agents incorrecte breaking changes rapporteren, vermoedelijk omdat de grens tussen een aanzienlijke verbetering en een potentieel brekende wijziging moeilijk correct te interpreteren is, zelfs wanneer de changelog beschikbaar is.

De resultaten ondersteunen de hypothese dat een AI-gestuurde analyselaag waardevol kan zijn voor dependencybeheer, maar benadrukken tegelijk dat de praktische meerwaarde niet uitsluitend bepaald wordt door het gebruikte taalmodel, maar ook door de beschikbaarheid van live changelog data, de kwaliteit van de outputstructuur en de robuustheid van de fallbacklogica bij ontbrekende releasenotes. Voor de context van Springbok betekent dit dat de keuze voor een implementatie een afweging vraagt tussen inhoudelijke kwaliteit, responssnelheid en onderhoudseenvoud.